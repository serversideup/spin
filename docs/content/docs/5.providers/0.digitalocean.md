...
[Sign Up with DigitalOcean →](https://m.do.co/c/f3bad4b927ca){target="_blank"}
...
To create an API token, you'll need to sign in to the [DigitalOcean Console](https://cloud.digitalocean.com/){target="_blank"}. On the left sidebar, click on the "API" section.
...
```yaml [.spin.yml configure providers]
##############################################################
...
```yaml [Example users configuration in .spin.yml]
##############################################################
...
```yaml [.spin.yml default Hardware Profiles]
##############################################################
# 🤖 Hardware Profiles
##############################################################
...
| `image` | The image to use for the server. | [DigitalOcean Images](https://docs.digitalocean.com/reference/api/api-reference/#tag/Images){target="_blank"} |
| `backups` | Whether to enable snapshot backups for the server (may add 20% to the monthly price). | Learn more about [DigitalOcean snapshot backups](https://www.digitalocean.com/products/backups){target="_blank"}. |

### Common Region Values
The `region` value is the `slug` of the region. You can find the full list of regions [here](https://docs.digitalocean.com/reference/api/api-reference/#tag/Regions){target="_blank"}.
...
### Common Size Values
::note
Make sure your server type is supported in the region you choose. Also, some of the pricing may change so be sure to check the latest pricing on the [DigitalOcean Droplet Pricing Page](https://www.digitalocean.com/pricing/droplets){target="_blank"}.
::

You can find the full list of server types [here](https://docs.digitalocean.com/reference/api/api-reference/#tag/Sizes){target="_blank"}.
...
```yaml [Example server configuration in .spin.yml]
##############################################################
