...
## Choose a host
We've run [VPS Benchmarks against a number of hosts](https://521dimensions.notion.site/VPS-Benchmarks-for-Self-hosters-c6eca7c5f16d4bb8aeb989174fc58ffe?pvs=4){target="_blank"} and have found the following hosts to have great value. The links below are referral links, meaning we get a small kickback if you sign up -- but this doesn't affect our recommendations.
...
[Sign Up with Digital Ocean →](https://m.do.co/c/f3bad4b927ca){target="_blank"}
...
- **RAM**: 1GB
- **Storage**: >25GB SSD
- **Architecture**: x86_64 (64-bit) [[request ARM64 support](https://github.com/serversideup/spin/discussions/9){target="_blank"}]
- **Operating System**: Ubuntu 22.04 LTS or newer ([**must be an LTS version**](https://ubuntu.com/about/release-cycle){target="_blank"})
