...
Installing `spin` at the project level is a great way to deploy `spin` without much configuration from the user's end. [View `spin` on NPM →](https://www.npmjs.com/package/@serversideup/spin){target="_blank"}

We can use Docker to run install `spin` on your project. **Run this command from the parent folder of your project.**

::note
The examples below are using `node:latest`. Be sure to change the image name to match your version of Node that you're using for your project. [See available versions →](https://hub.docker.com/_/node/){target="_blank"}
::

```bash [Install "spin" with Yarn + Docker]
docker run --rm -v  $(pwd):/usr/app/src -w /usr/app/src/ node yarn add @serversideup/spin --dev
```

```bash [Install "spin" with NPM + Docker]
...
```bash [Run "spin" from your project]
...
```bash [Add to your shell profile (recommended)]
...
```bash [Initialize Spin with your project]
spin init
```