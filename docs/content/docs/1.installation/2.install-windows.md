...
* [Windows Terminal](https://docs.microsoft.com/en-us/windows/terminal/install){target="_blank"} - This is a great terminal application that will give you tab support for your CLIs.

## Install Windows Subsystem Linux v2 (WSL2)
Both options require installing WSL2. We can follow the principles discussed in the [official WSL2 documentation](https://docs.microsoft.com/en-us/windows/wsl/install){target="_blank"} and prepare our machine to run "Windows Subsystem Linux v2" (WSL2). We prefer Ubuntu as our distribution. So if you're happy with that recommendation, run this command in PowerShell as administrator to install WSL2.

```bash [Install and use Ubuntu as your Windows Subsystem Linux distro]
...
If you prefer to install Docker Desktop with the WSL2 backend, you'll need to [download Docker Desktop](https://www.docker.com/products/docker-desktop){target="_blank"} from Docker's website.
...
```bash [Validate Docker is working (run this in Windows Terminal)]
docker info
```
...
```bash [Run a simple container to ensure Docker is working]
