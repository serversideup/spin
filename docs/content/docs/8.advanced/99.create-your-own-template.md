...
```bash [Use the "serversideup/spin-template-laravel-basic" template]
...
You can check out the [serversideup/spin-template-laravel-basic](https://github.com/serversideup/spin-template-laravel-basic){target="_blank"} project to get a feel for how to structure your repository. Here are some key points:
...
[See our install.sh file](https://github.com/serversideup/spin-template-laravel-basic/blob/main/install.sh){target="_blank"} for the serversideup/spin-template-laravel-basic template for an example.
...
You can review [our init source code](https://github.com/serversideup/spin/blob/main/lib/actions/init.sh){target="_blank"} to see exactly when this file is executed.
...
When Spin runs your `install.sh` file, you have access to a number of functions to help you get started. Any function from our [functions.sh](https://github.com/serversideup/spin/blob/main/lib/functions.sh){target="_blank"} file is available to be called, but here are our favorites:

### `add_user_todo_item`
If a user needs to take action after installing your template, you can show a "todo" to the user for them to take action.

```bash ["add_user_todo_item" example]
...
```
```bash [Operate on multiple files]
...
We're working on a way to list community templates on the Spin website. If you're interested in having your template listed, [please open a Discussion on GitHub](https://github.com/serversideup/spin/discussions){target="_blank"} and we'll get you added to the list.