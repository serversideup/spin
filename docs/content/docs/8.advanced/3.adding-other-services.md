...
```yml ["Example: docker-compose.yml"]
services:
  mariadb: 
    image: mariadb:11.4
```

```yml ["Example: docker-compose.dev.yml"]
services:
  mariadb:
    networks:
      - development
...
```yml ["Node Traefik Labels Example"]
    labels:
      - "traefik.enable=true"
...
## PHP
We use our open source PHP Docker Images, which are based on the official PHP images, but production-ready by default and optimized for Laravel out of the box. Read our guide on [selecting the right image variation](https://serversideup.net/open-source/docker-php/docs/guide/choosing-the-right-image){target="_blank"} for your project.
...
```yml ["Laravel Traefik Labels Example"]
    labels:
      - "traefik.enable=true"
...
## Soketi
See the [Soketi documentation](https://docs.soketi.app/getting-started/installation/docker){target="_blank"} for selecting the correct version.
```yml ["Example: docker-compose.yml"]
services:
  socket:
...
```yml ["Example: Development configuration at `./.infrastructure/conf/traefik/dev/traefik.yml`"]
...
```yml ["Example: Development configuration at `./.infrastructure/conf/traefik/dev/traefik-certs.yml`"]
tls:
  stores:
    default:
      defaultCertificate:
...
If you use `spin init` or `spin new` to create your project, we ship a `local-dev` keypair. This is signed by the Server Side Up Certificate Authority. If you'd like to trust this CA, you need to install the CA Root on your machine (only do this if you trust our process): [https://serversideup.net/ca/](https://serversideup.net/ca/){target="_blank"}.