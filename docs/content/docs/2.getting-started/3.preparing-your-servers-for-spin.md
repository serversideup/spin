...
::note
It is best to run `spin provision` on a **brand new server** with a fresh installation of Ubuntu 22.04 or newer. This will ensure that the server is in a known state and that the configuration will work as expected.
::
...
- **Firewall**: Configured to only allow SSH and HTTPS traffic (22/tcp, 80/tcp, 443/tcp, 5080/tcp)
