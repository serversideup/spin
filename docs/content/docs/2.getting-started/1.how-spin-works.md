...
- [Docker Compose](https://docs.docker.com/compose/){target="_blank"} (used in local development)
- [Docker Swarm Mode](https://docs.docker.com/engine/swarm/){target="_blank"} (used in production, staging, etc.)
- [Ansible](https://www.ansible.com/){target="_blank"} (used to ensure your server is configured correctly)
...
#### Ansible Collection
- [Spin Ansible Collection](https://github.com/serversideup/ansible-collection-spin){target="_blank"} - Used to provision and maintain your production server.

#### GitHub Actions
- [serversideup/docker-build-action](https://github.com/marketplace/actions/docker-build-action){target="_blank"} - A simplified syntax to build and publish your Docker images with GitHub Actions.
- [serversideup/docker-swarm-deploy-github-action](https://github.com/marketplace/actions/docker-swarm-deploy-github-action){target="_blank"} - A simplified syntax to deploy to Docker Swarm Mode via GitHub Actions.

#### Docker Images
- [serversideup/php](https://serversideup.net/open-source/docker-php/){target="_blank"} - PHP Docker images highly optimized to work with Laravel + Spin.
- [serversideup/docker-ssh](https://github.com/serversideup/docker-ssh){target="_blank"} - A lightweight docker image that runs SSH. This is a fantastic method on using a secure SSH tunnel into your database cluster.
- [serversideup/docker-ansible](https://github.com/serversideup/docker-ansible){target="_blank"} - A lightweight docker image that runs Ansible. This allows users to provision their server without needing to know how to install or maintain their Ansible installation.
- [serversideup/docker-github-cli](https://github.com/serversideup/docker-github-cli){target="_blank"} - A lightweight docker image that runs GitHub CLI. This allows users to interact with GitHub without needing to install the GitHub CLI on their machine.
...
## Important concepts
Spin's flexibility comes from embracing the [Docker Compose Overrides (aka Docker Compose Extends)](https://docs.docker.com/compose/multiple-compose-files/extends/){target="_blank"} structure. We use a "base" `docker-compose.yml` file, then choose a file based on the environment.
...
```bash [What we would have to type with Docker Compose 😅]
...
```yaml [If we were to run "spin up", our expected result of Docker Compose that we'll run]
services:
  php:
